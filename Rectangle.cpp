...
{
    return (contains(x - width / 2.0f, y - height / 2.0f) && contains(x + width / 2.0f, y + height / 2.0f));
}
...
{
    float t;
    if ((t = x - this->x) > this->width / 2.0f || -t > width / 2.0f)
        return false;
    if ((t = y - this->y) > this->height / 2.0f || -t > height / 2.0f)
        return false;
    return true;
}
